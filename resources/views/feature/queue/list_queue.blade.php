        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu p-3" style="min-width:200px">
                ...
                <form action="{{ route('queue.take') }}" method="POST">
                    @csrf
                    <div class="d-flex gap-2">
                        <button name="category" value="A" class="btn btn-sm btn-outline-primary flex-fill">A</button>
                        <button name="category" value="B" class="btn btn-sm btn-outline-warning flex-fill">B</button>
                        <button name="category" value="C" class="btn btn-sm btn-outline-success flex-fill">C</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <li class="list-group-item d-flex align-items-center justify-content-between py-3 px-4">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <form action="{{ route('queue.counter.toggle', $counter) }}" method="POST">
                                @csrf @method('PATCH')
                                <button class="btn btn-sm {{ $counter->status === 'closed' ? 'btn-outline-success' : 'btn-outline-secondary' }}" title="{{ $counter->status === 'closed' ? 'Open' : 'Close' }}">
                                    <i class="bx {{ $counter->status === 'closed' ? 'bx-power-off' : 'bx-x' }}"></i>
                                </button>
                            </form>
                        </div>
                    </li>
...
<script>
    ...
    setInterval(function() {
        $('#queueTableBody').load(
            window.location.href + ' #queueTableBody>*'
        );
    }, 3000);
